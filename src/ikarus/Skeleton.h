...
public:
	...
	vec3d getJointPos(Joint &j);

	const Joint &getRoot() const;
	const vec3d &getRootPos() const;
	void setRoot(Joint &j);
	void setRootPos(const vec3d &pos);
	const JointState &getState(const Joint &j);
private:
	Skeleton *skeleton;
	...
	void updateJointPos(Bone *fromBone, Joint &j, const mat4d &basis);
	void updateBoneJointPos(Joint *fromJoint, Bone &b, const mat4d &basis);
